...
def mul_relu_block_back_kernel(
    ...
    # Finish me!
    i_offset = tl.arange(0, B0) + block_id_i * B0
    j_offset = tl.arange(0, B1) + block_id_j * B1
    x_offset = j_offset[:, None] * N0 + i_offset
    i_mask = i_offset < N0
    j_mask = j_offset < N1
    x_mask = j_mask[:, None] & i_mask[None, :]
    x = tl.load(x_ptr + x_offset, mask=x_mask)
    y = tl.load(y_ptr + j_offset, mask=j_mask)
    dz = tl.load(dz_ptr + x_offset, mask=x_mask)
...
def softmax_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    # max and exp sum
    max = tl.full((B0, 1), -float("inf"), dtype=tl.float32)
    ...
    for start_j in tl.range(0, T, B1):
        ...
        exp_sum = exp_sum * factor + exp_x_sum[:, None]
        max = x_max
    ...
    for start_j in tl.range(0, T, B1):
        ...
        mask_ij = mask_i[:, None] & mask_j
        x = tl.load(x_ptr + off_ij, mask=mask_ij)
        softmax_x = tl.exp2(log2_e * (x - max)) / exp_sum
        tl.store(z_ptr + off_ij, softmax_x, mask=mask_ij)
...
def flashatt_kernel(
    ...
    print(f"o shape: {o.shape}, o trans shape: {tl.trans(o).shape}, o trans2 shape: {o[None, :].shape}")
    tl.store(z_ptr + off_i, tl.trans(o), mask=mask_i)
